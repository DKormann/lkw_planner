// import { type ScheduleItem, type UUID, ScheduleStep, Request, Schedule, Module } from "../types";
// // import { getCostN, optDur, optimizeSchedule, rateSchedule } from "../planner";
// import { mkWritable, type Writable } from "../writeable";
...
// export function getRequest(mod: Module, id: UUID){
//   let req = mod.requests.find(r=>r.id == id)
//   if (!req) throw new Error(`not found request ${id}`)
//   return req
// }

// export function stepRequest(mod: Module, step: ScheduleStep){
//   if (step.$ == "start") return undefined
//   return getRequest(mod, step.val.request)
// }

// function stepString (mod: Module, step: ScheduleStep){

//   if (step.$ == "start") return `start`
//   let req = getRequest(mod, step.val.request)
//   return `${step.$} ${requestString(step.val.request)}: ${costString(req.value_eur)} deadline ${timeString(req.deadline_h)}`
// }
...
//     else return
//     e.preventDefault()
//     cursor.row = Math.max(0, Math.min( schedule_store.get().length-1, cursor.row))
//     cursor.col = Math.max(0, Math.min( schedule_store.get()[cursor.row]!.steps.length-1, cursor.col))
//   })
...
// export const scheduleView = (schedule_store:Writable<Schedule>) => {
...
//             background:req && req.deadline_h < time ? color.red : "",
...
//   let value = span()
//   schedule_store.onupdate(sch=>value.textContent = rateSchedule(sch).toFixed(2))
...
//   let steps = schedule_store.get()[row]
//   if (!steps) return
...
//   let dead = step.$ != "start" && getRequest(step.val.request).deadline_h < dist
