...
var isStmt = (x) => !!x && typeof x === "object" && ("kind" in x) && (x.kind === "local.set" || x.kind === "array.store" || x.kind === "array.move" || x.kind === "block" || x.kind === "loop" || x.kind === "break" || x.kind === "continue" || x.kind === "return" || x.kind === "call.void" || x.kind === "trap" || x.kind === "log" || x.kind === "expr" || x.kind === "if" && Array.isArray(x.then));
...
var allocateLocal = (type) => localExpr(type, nextLocalId++);
var mkLocal = (type) => {
...
var mkStructArray = (type, length) => {
  ...
  handle = {
    ...
    at: (index) => structValue(type, memoryValue(handle, index, type.storage, type.size)),
    move: (target, source, count) => ({ kind: "array.move", array: handle, target: lit("i32", target), source: lit("i32", source), count: lit("i32", count) })
  };
  return handle;
};
...
function i32(value) {
  return typeof value === "number" ? expr({ kind: "const", type: "i32", value }) : cast("i32", value);
}
function i64(value) {
  return typeof value === "bigint" ? expr({ kind: "const", type: "i64", value }) : cast("i64", value);
}
...
var walkStmt = (s, fns) => {
  switch (s.kind) {
    ...
    case "call.void":
      fns.func?.(s.target);
      s.args.forEach((arg) => walkExpr(arg, fns));
      return;
    case "expr":
      walkExpr(s.expr, fns);
      return;
...
var walkBody = (body2, visitors) => {
  const stmts = asStmts(body2);
  stmts ? stmts.forEach((s) => walkStmt(s, visitors)) : walkExpr(body2, visitors);
};
...
var moduleFuncs = (mod2) => Object.fromEntries(Object.entries(mod2).filter(([, v]) => v.kind === "func"));
var moduleArrays = (mod2) => Object.fromEntries(Object.entries(mod2).filter(([, v]) => v.kind === "array"));
var buildFunc = (func2) => {
...
var analyzeModule = (mod2) => {
  const funcs = moduleFuncs(mod2);
  const arrays = moduleArrays(mod2);
...
var cmpOps2 = ["eq", "lt", "gt"];
var resultType = (result) => typeof result === "object" ? result.storage === "i64" ? "i64" : "i32" : result;
var offsetCodes = (offsets, bases) => Object.fromEntries(Object.entries(offsets).map(([op, offset]) => [
  op,
  Object.fromEntries(Object.entries(bases).map(([type, base]) => [type, base + offset]))
]));
var arithmeticCodes = offsetCodes({ add: 0, sub: 1, mul: 2, div: 3 }, { i32: 106, i64: 124, f32: 146, f64: 160 });
var bitCodes = offsetCodes({ and: 7, or: 8, xor: 9, shl: 10, shr: 12 }, { i32: 106, i64: 124 });
var remainderCodes = offsetCodes({ mod: 5, umod: 6 }, { i32: 106, i64: 124 });
var signedCmpCodes = offsetCodes({ eq: 0, lt: 2, gt: 4 }, { i32: 70, i64: 81 });
var floatCmpCodes = offsetCodes({ eq: 0, lt: 2, gt: 3 }, { f32: 91, f64: 97 });
var compareCodes = Object.fromEntries(cmpOps2.map((op) => [op, { ...signedCmpCodes[op], ...floatCmpCodes[op] }]));
var codes = {
...
var compileExpr = (e, fix, lix, arrays, options) => {
  switch (e.kind) {
    ...
    case "call":
      return [...flatMap(e.args, (arg) => compileExpr(arg, fix, lix, arrays, options)), 16, ...u32(fix.get(e.target) + 2)];
    case "cast": {
    ...
    case "load": {
      ...
      return [...runtimeBoundsGuard(e.index, layout.length, fix, lix, arrays, options), ...compileExpr(addr(layout, e.index, e.stride, e.offset), fix, lix, arrays, options), codes.load[e.storage], ...memarg(e.storage)];
    }
    default:
      return die2(e);
  }
...
var compileStmt = (s, fix, lix, arrays, options, traps, logs, stack = []) => {
  switch (s.kind) {
    case "local.set":
    ...
    case "if":
      return [...compileExpr(s.cond, fix, lix, arrays, options), 4, 64, ...flatMap(s.then, (x) => compileStmt(x, fix, lix, arrays, options, traps, logs, [{}, ...stack])), ...s.else.length ? [5, ...flatMap(s.else, (x) => compileStmt(x, fix, lix, arrays, options, traps, logs, [{}, ...stack]))] : [], 11];
    case "block":
      return [2, 64, ...flatMap(s.body, (x) => compileStmt(x, fix, lix, arrays, options, traps, logs, [{ control: s.control, kind: "break" }, ...stack])), 11];
    case "loop":
      return [2, 64, 3, 64, ...compileExpr(s.cond, fix, lix, arrays, options), 69, 13, ...u32(1), ...flatMap(s.body, (x) => compileStmt(x, fix, lix, arrays, options, traps, logs, [{ control: s.control, kind: "continue" }, { control: s.control, kind: "break" }, ...stack])), 12, ...u32(0), 11, 11];
    case "break":
      if (s.target == null)
    ...
    case "return":
      return [...s.value ? compileExpr(s.value, fix, lix, arrays, options) : [], 15];
    case "trap":
      return [65, ...sN(traps.get(s.message), 32), 16, 0];
    case "log":
      return [65, ...sN(logs.get(s.message), 32), ...compileExpr(s.value, fix, lix, arrays, options), 16, 1];
    case "call.void":
      return [...flatMap(s.args, (arg) => compileExpr(arg, fix, lix, arrays, options)), 16, ...u32(fix.get(s.target) + 2)];
    case "expr":
...
var emitModule = ({ fEntries, builtFuncs, fix, layouts, trapMessages, logMessages, pages }, options = {}) => {
  ...
  return new Uint8Array([
    ...
    ...section(10, [
      ...
      ...flatMap(builtFuncs, ({ func: func2, built, locals, localIndexes }) => {
        ...
        const result = resultType(func2.result);
        const code = stmts ? [...flatMap(stmts, (s) => compileStmt(s, fix, localIndexes, layouts, options, traps, logs)), ...result === "void" ? [] : codes.zero[result]] : compileExpr(built, fix, localIndexes, layouts, options);
...
var compile = async (mod2, options = {}) => {
  ...
  const log = (id, value) => console.log(analysis.logMessages[id] ?? `WASM log ${id}`, value);
  const instance = await WebAssembly.instantiate(compiled, { env: { memory, trap: trap2, log } });
  const funcEntries = Object.entries(analysis.funcs);
  const resultStructs = Object.fromEntries(funcEntries.flatMap(([name, func2]) => typeof func2.result === "object" ? [[name, func2.result]] : []));
  const jsFuncs = Object.fromEntries(funcEntries.map(([name, func2]) => {
    const wasmFunc = instance.exports[name];
    if (typeof func2.result !== "object")
      return [name, wasmFunc];
    const result = func2.result;
    return [name, (...args) => decodeStruct(result, wasmFunc(...args))];
  }));
  const jsArrays = Object.entries(analysis.arrays).map(([name, arr]) => {
    ...
    if (typeof arr.type !== "string") {
      ...
      return [name, new Ctor2(memory.buffer, layout.offset, arr.length)];
    }
...
//# debugId=46C7A80BE499D51864756E2164756E21
//# sourceMappingURL=data:application/json;base64,
